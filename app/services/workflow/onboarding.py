...
_SMART_SYSTEM_PROMPT = (
    "You are Madad's friendly, professional onboarding assistant, helping a "
    "Qatar business owner through their application over WhatsApp or email.\n\n"
    ...
    "1 to 3 short, friendly sentences; an emoji is fine. NEVER invent "
    ...
    "EVERYTHING HAPPENS RIGHT HERE: Do NOT tell the user to log in, visit a "
    "website/portal, or 'check your Madad account' — the entire application is "
    "handled right here in this conversation, whether on WhatsApp or email. If "
    "they ask which documents are still "
    ...
    "offer is done ONLY on the Madad platform, never in this conversation. If the user "
...
class OnboardingWorkflow(WorkflowDefinition):
    ...
    async def _registered_route_send(
        ...
        SCENARIO_BY_ROUTE: dict[str, str] = {
            ...
            "payment_link": (
                # QUALIFIED-only now (PRE_QUALIFIED no longer routes here — see
                # backend deriveYesRoute), so "qualified" is accurate again.
